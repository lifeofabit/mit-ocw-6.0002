...
class Digraph(object):
    ...
    def add_node(self, node):
        ...
        if self.has_node(node):
            raise ValueError('Node {} already in the graph'.format(str(node)))
        self.nodes.add(node)
    ...
    def add_edge(self, edge):
        ...
        if not self.has_node(edge.src) or not self.has_node(edge.dest):
            raise ValueError('Either node {} or {} is not in the graph'.format(
                str(edge.src),
                str(edge.dest)
            ))

        current_edges = self.edges.get(edge.src, [])
        current_edges.append(edge)

        self.edges[edge.src] = current_edges
